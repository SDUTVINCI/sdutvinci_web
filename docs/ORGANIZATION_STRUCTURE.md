...
公开页面使用代码原生的全页动态“星系”布局，不依赖一张固定图片。中心“协同运行”保持固定，三
机构使用官方 Logo 沿内层轨道缓慢公转，Logo 与文字始终正向；全部 `group` 节点作为部门行星在
更大的外环运行，不再固定排成左右两列。部门周围的小卫星代表其 `role` 数量，点击部门行星会暂停
整个部门外环、弱化其他部门，并打开局部轨道聚焦舱，完整显示部门名称及每个岗位名称；没有岗位
时明确显示“当前未配置下级岗位”。负责人层保持固定，机器人队队长继续位于正中并突出显示；新闻
部与运营组关系使用底部彗星轨迹表达。聚焦舱可用关闭按钮或 `Esc` 退出。
...
这个映射只决定展示素材，不进入组织架构 JSON，也不改变数据库结构。深色与浅色页面使用各自的
星空背景、文字、轨道和玻璃面板变量，避免把浅色白底直接带入深色模式。窄屏和 CMS 紧凑预览会
停止公转并切换为可滚动语义卡片；窄屏点击部门时使用居中浮层显示局部星系。
`prefers-reduced-motion: reduce` 会停止机构、部门、卫星、星光和轨迹动画，但不影响点击查看岗位。
...
公开页点击任一部门行星即可查看该部门的岗位局部轨道；再次选择其他部门会切换聚焦内容，关闭按钮
或 `Esc` 返回完整星系。此交互只读取现有 `group → role` 树，不引入额外展示配置或成员关联。
