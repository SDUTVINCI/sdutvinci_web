...
深色模式使用近白星光与少量青蓝色，浅色模式将同一星流转换为较低透明度的蓝青星尘；组织图面板
降低整块遮罩强度，让星流在图内外连续可见。渲染像素总量有上限，动画约 30 帧每秒；页面进入后台
时暂停循环，开启减少动态效果时只绘制静态帧。主题属性、系统主题或容器尺寸变化都会触发重新绘制，
WebGL 不可用时提供静态多层星点回退。
