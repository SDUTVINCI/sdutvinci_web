...
公开页的最底层使用 `OrganizationGalaxyBackground.vue` 绘制程序化星空。实现直接按
[React Bits Galaxy](https://reactbits.dev/backgrounds/galaxy) 的四层 WebGL 星流 shader 语义移植为
Vue 组件，并使用 `ogl` 创建全屏三角形，不引入 React。鼠标离开组织页面时显示均匀的高密度星场；
进入后星点会围绕指针产生平滑排斥、景深拉伸与穿梭中心，离开时再缓动恢复。背景容器设置
`pointer-events: none`，实际指针坐标由页面层监听，因此不会遮挡部门、负责人或关系线交互。
深色模式使用近白星光与少量青蓝色；浅色模式用更高密度、低耀光的冷灰蓝细星尘层叠加少量亮星
核心，保持接近深色模式的星点数量，同时避免在白底上形成大块深蓝光斑。组织图面板降低整块遮罩
强度，让星流在图内外连续可见；浅色模式单独提高主椭圆、信号虚线与机构局部轨道的对比度，并用
机构强调色区分局部系统，避免亮星流穿过时吃掉轨迹。
桌面端最高 120 FPS，粗指针设备最高 60 FPS，实际绘制仍由 `requestAnimationFrame` 约束在屏幕
刷新率内，因此 60Hz 桌面不会额外计算 120 帧。指针平滑按实际帧间隔计算，在不同帧率下都约
0.2 秒收敛，排斥强度进入快、离开缓。渲染像素总量有上限，页面进入后台时暂停循环，开启减少
动态效果时只绘制静态帧。
主题属性、系统主题或容器尺寸变化都会触发重新绘制，WebGL 不可用时提供静态多层星点回退。
