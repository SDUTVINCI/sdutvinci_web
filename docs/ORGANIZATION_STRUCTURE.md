...
公开页的最底层使用 `OrganizationGalaxyBackground.vue` 绘制程序化星空。实现参考
[React Bits Galaxy](https://reactbits.dev/backgrounds/galaxy) 的多层景深星点、缓慢漂移、闪烁和
指针视差体验，但使用 Vue 与原生 2D Canvas 独立实现，
不复制 React/OGL 组件，也不增加运行依赖。Canvas 覆盖整个组织页面，星图面板使用半透明表面让
背景连续穿过英雄区与组织图；它设置 `pointer-events: none`，不会遮挡部门、负责人或关系线交互。
深色模式使用深蓝黑背景与青蓝/少量暖色星点，浅色模式使用冰蓝灰背景与低对比蓝绿星尘，避免白色
大块和文字对比不足。设备像素比被限制，动画约 30 帧每秒；页面进入后台时暂停循环，开启减少动态
效果时只绘制静态帧。主题属性、系统主题或容器尺寸变化都会触发
重新绘制。
