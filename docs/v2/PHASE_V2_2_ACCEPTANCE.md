...
- 阶段总体完成：已勾选。
- 验收限制：人工验收数据库曾被错误连接的破坏性自动测试覆盖，因此切回
  `legacy_git` 后“已登录 Revision API 返回 404”没有重新人工复验；维护者明确接受
  此限制。服务运行模式、已有 Wiki、测试 Git 工作区/远端一致性和开关断言已验证。
...
`test:backup-restore` 会在创建隔离 Compose project 前清除调用终端中的应用、数据库、
CMS Git、S3/COS 和 Compose 覆盖变量，再只使用测试脚本生成的临时 `.env`。输出中的
project、数据库、端口、镜像和路径都必须带 `phase9`/`test` 标识；发现其他目标立即
中止。退出时会清理精确匹配本次动态标签的测试镜像，以及容器、volume 和临时目录。
...
- 维护者结论：通过，接受已记录的登录后 Revision API 404 复验限制。
- 人工验收日期：2026-07-29。
- 阶段 2 实现 Commit：`7cdc8c330042e10a1810b5b784ff38fc63ea007e`。
- 验收修复 Commits：`4a2085b24f324bba060ab2ee6fb3e42a9e0f3a0e`、
  `d3cd75ee4226f70b948eab3e367b40978c37f5d4`、
  `3caf82b4430bbc12a59c69222886501f85c605b1`、
  `cc8ff9b36f88c32131d158f85c26bb2b1503f7f8`。
- 维护者确认原文：`V2 阶段 2 验收通过，接受已记录的登录后 Revision API 404 复验限制。`
- 阶段 3 状态：实现已完成，等待维护者人工验收。
