...
列表，不改变已经勾选的项目、数据库状态或权限。执行导入时若同一账号已有该文章的活动草稿
（草稿、待审核、已驳回、已通过或已撤回），项目会记录 `IMPORT_ACTIVE_DRAFT_EXISTS`、冲突草稿
ID 与状态，并显示中文原因和草稿入口。`published` 属于已发布历史，不是活动草稿；导入会在同一
事务中复用该记录、更新为 PR 内容并重新打开为草稿，不会因历史记录误阻止，也不会违反同账号
同文章仅一条未删除记录的唯一约束。因活动草稿而 blocked 的项目允许重新勾选重试，适用于处理
或删除冲突草稿后的原导入报告；成功时会清除旧阻止码和冲突详情。历史上未记录具体错误码的
其他 blocked 项显示明确的旧记录说明，不再把“安全修改”分类误解成已经成功导入。

外部内容导入页提供“全选当前可导入结果”：选择当前风险分类与处理状态筛选结果中仍为 pending，
或仅因 `IMPORT_ACTIVE_DRAFT_EXISTS` 阻止而可重新检查，且服务端允许普通导入或高风险人工确认的
项目；其他已阻止、已导入、已跳过及不可覆盖的冲突项不会被选中。取消全选也只移除当前结果，
保留其他筛选条件下已经手动勾选的项目。草稿页
对应“全选可提交草稿”，审核页分别对应“全选待审核”和“全选待发布”。

审核页把“待审核文章”“已通过，等待发布”和“成员信息申请”统一为三个连续流程面板。每个面板
在同一标题栏显示阶段、当前数量与相关批量操作；没有数据时使用紧凑空状态，不保留空表格和悬空
操作栏。窄屏下操作按钮自动换为等宽双列或单列，表格仍可横向滚动，批量接口、确认短语、逐项
结果和权限边界不变。
