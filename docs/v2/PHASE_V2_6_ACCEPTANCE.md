...
> 状态：维护者已于 2026-07-30 明确回复“V2 阶段 6 验收通过”，阶段正式收尾。本文
> 只覆盖阶段 6；没有
...
故障注入时，数据库 Revision #4 和前台正文立即生效；远端保持旧成功 Commit
`52e8006a6efebbb346653bfacf6ae1600d3203c9`，任务两次失败后进入 `failed`。维护者在
远端修复前提前点击了手动重试；系统安全记录一条 `content_export.retry` 审计及
`previousAttemptCount: 2`，重置预算后再次尝试两次并回到 `failed`，数据库仍未回滚。

该截图发现浏览器虽已遮盖远端 URL，却仍显示 Git 命令和合成远端 stderr，超过 CMS
所需诊断范围。运行/job 审计继续保留已脱敏运维详情；CMS 状态改为仅按错误码返回固定
安全摘要，不再返回存储的 Git 错误文本。新增负向断言拒绝 `git push` 和
`pre-receive` 暴露；修复后阶段 6 专项 9/9、完整 CMS 90/90、typecheck 和 build
通过。应用原地重启后仍保留 failed job、重试审计、管理员、远端和 Worker。维护者需
刷新确认安全摘要，再恢复远端并重新手动重试。

维护者确认安全摘要后，隔离拒绝钩子按归属标记移除。最终手动重试成功，任务新一轮
尝试 1 次，远端前进到
`7cc16a05caca1a0619116d6256e43b1a33e55e42`；两条人工重试审计均保留
`previousAttemptCount: 2`。目标 Markdown 包含稳定 `vinciId` 和“数据库不受 Git
影响”，snapshot 指向 Revision #4，snapshot/manifest 中目标 SHA-256 均为
`c0aded2177f7663d3bb6d5334d587141c4de01ba6f4c469ac7a31705f37994e0`，
最终 `issueCount: 0`。维护者明确回复“V2 阶段 6 验收通过”后，数据库、应用、
Worker、本地裸远端、workspace、日志和临时根均已精确清理。
...
- [x] 维护者浏览器人工验收完成。
- [x] 维护者明确允许阶段 6 收尾。
- 阶段 6 实现 Commit：`d3528bf`。
- 人工验收修复 Commit：`a535ce2`、`18baf8a`、`09369b3`。
- 阶段 6 验收记录 Commit：由最终回复报告，不在 Commit 内预填自身 SHA。
