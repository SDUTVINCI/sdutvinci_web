...
> 状态：实现和自动验证完成，等待维护者浏览器验收。本文只覆盖阶段 5；没有启动
> Outbox Worker、连接独立内容仓库、Push 或部署。人工项在维护者明确回复
> “V2 阶段 5 验收通过”前保持未勾选。
...
CommonMark 图片节点；可视化初始化的“无损往返检查”同时改为 fail closed：除换行、
行尾空白和首尾空白规范化外，任何实质序列化变化都会保留原文并自动退回源码模式。
回归测试覆盖中文图片说明被替换为 `1.00`，真实无头 Chrome 往返也确认说明原样保留。
...
- [ ] 我在隔离浏览器环境发布文章并立即刷新数据库前台，确认内容生效。
- [ ] 我确认无效测试 Git 远端下发布仍成功。
- [ ] 我确认后台显示当前 Revision 和“等待导出”。
- [ ] 我测试多人旧 `base_revision_id` 冲突。
- [ ] 我查看历史、详情、Diff，并确认恢复追加新 Revision。
- [ ] 我测试删除与恢复删除。
- [ ] 我测试四开关 Git-first 回滚和重新切回数据库。
- [ ] 我保存失败证据或确认 `issueCount: 0`，并精确清理资源。
- [ ] 我明确回复“V2 阶段 5 验收通过”。
