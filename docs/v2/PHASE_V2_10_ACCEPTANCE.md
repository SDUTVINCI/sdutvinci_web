...
标签未 Push，未覆盖既有标签。恢复包位于带 marker 的本地临时根；人工验收结束后才精确清理：
...
- 回滚标签、tar、bundle、SHA 清单、恢复数据库和隔离运行环境在人工验收前保留，便于取证。
...
- 验收结论：等待维护者验收。
- 维护者确认时间：
- 实现 Commit：由最终交付回复报告完整 SHA，避免在 Commit 内自引用。
- 验收修复 Commit：
- 维护者确认原文：

维护者完成上述步骤并接受证据后，请明确回复：`V2 阶段 10 验收通过`。在此之前不得勾选
阶段 10 人工项、阶段总体完成项，也不得进入阶段 11。
