...
- 实现状态：完成，自动化验证通过。
- 人工验收：维护者已于 2026-07-29 明确验收通过。
- 阶段总体完成：已勾选。
- 验收结论：接受 33 篇/35 项兼容差异留到阶段 4 影子对比，并确认未切换生产前台、
  内容权威或发布事务。
...
- 阶段 2 已于 2026-07-29 完成人工验收；已登录 Revision API 404 的复验限制见阶段 2
  验收记录，维护者已明确接受。
...
自动测试数据库必须与阶段 2/3 人工验收数据库分离；集成测试会清空 CMS 业务表，
`TEST_DATABASE_URL="$DATABASE_URL"` 被明确禁止并由测试护栏拒绝。
...
阶段 2 已验收且阶段 3 自动验证全部通过时，维护者无需重复手工执行锁、上传失败、XSS
夹具和全量测试。使用全新的 `phase3_acceptance_test` 数据库启动本地开发服务后，只需
在浏览器抽查两篇现有 Wiki：

1. `C++基础初识`：检查可视化编辑、Markdown 源码、最终效果预览，做一处临时文本
   修改并保存、刷新核对，再删除临时修改；不得提交审核或发布。
2. `Ignition Gazebo（Gazebo Fortress）`：检查代码、表格、图片、目录、链接、
   `<NuxtLink>` 和原始 HTML 在最终预览中未被静默吞掉。

最后确认接受兼容报告中的 260/260 解析成功、0 渲染失败以及 33 篇/35 项待阶段 4
继续判断的差异，再停止服务并删除精确命名的临时数据库。此精简路径覆盖最高优先级
需求第 19.5 节；锁、自动保存时序、三模式上传和 XSS 边界由第 19.4 节已通过的自动化
验证承担，不把它们伪造成重复人工操作。
...
- [x] 我已完成阶段 2 的人工验收，并接受验收记录中的明确限制。
...
- [x] 我已确认生产前台、内容权威和发布事务没有改变。
- [x] 我已明确回复“V2 阶段 2 验收通过”。
- [x] 我已明确回复“V2 阶段 3 验收通过”。

## 12. 人工验收记录

- 人工验收日期：2026-07-29。
- 阶段 3 实现 Commit：`90c2d58761847e72b4e354f1b85881e15a0c0a8c`。
- 维护者确认原文：`V2 阶段 3 验收通过；我接受 33 篇/35 项差异留到阶段 4 影子对比，并确认未切换生产前台、内容权威或发布事务。`
- 阶段 4 状态：尚未开始，等待新对话按最高优先级需求重新复核基线。
- 发布状态：未 Push、未部署。
