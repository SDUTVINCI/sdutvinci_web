...
- [x] 我在隔离测试环境切换并检查新闻数据库来源。
- [x] 我在隔离测试环境切换并检查 Wiki 数据库来源。
- [x] 我检查桌面端和手机端。
- [x] 我检查目录、锚点、上一页、下一页、图片放大和阅读进度。
- [x] 我抽查 33 篇/35 项及额外 25 条报告差异。
- [x] 我确认关闭开关后旧前台恢复。
...
- 阶段 4 实现 Commit：
  `9a64eb38af952b999247cee864d759a3573b6932`
- 验收结论：通过。
- 维护者确认时间：2026-07-29。
- 维护者确认原文：`V2 阶段 4 验收通过`
- 验收记录 Commit：由本次最终回复报告，不在 Commit 内预填自身 SHA。
- 两个 Commit 均不 Push、不部署；本次只完成阶段 4 记录，不开始阶段 5。
