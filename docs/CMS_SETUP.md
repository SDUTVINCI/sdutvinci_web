...
本文覆盖 PostgreSQL、身份认证、成员管理、文章只读索引、Markdown 编辑器和数据库草稿。审核、发布和图片上传尚未开放。
...
后台登录地址为 `http://localhost:3000/cms/login`，使用账号 ID 和密码登录，不使用邮箱登录。登录后可访问工作台、文章、草稿、成员列表及个人资料；管理员可以创建和维护成员姓名、头像。管理员用户管理 API 位于 `/api/cms/admin/users`，用户管理界面将在后续阶段完善。
...
- 可视化模式基于 Milkdown Crepe；原始 HTML、Vue 组件、Jekyll/MDC 等扩展语法只允许使用源码模式。
- 本阶段没有提交审核、正式发布、Git 操作或图片上传按钮。
