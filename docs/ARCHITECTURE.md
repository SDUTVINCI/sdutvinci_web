...
匿名用户直接打开真实存在的受限详情时跳转 CMS 登录，并在登录成功后回到原文章；真实不存在、
已删除或非法路径仍返回 404。列表、搜索、Sitemap 和 RSS 不因此泄露受限文章。
Wiki 首页是列表规则的唯一例外：匿名用户可看到按文档根合并的受限 Wiki 标题卡片，用于发现并
登录；接口不提供受限章节标题、数量、正文、Revision、哈希或摘要。混合公开/受限 Wiki 只在公开
目录显示公开页面，并明确标注“部分内容需登录”和登录入口；仍不显示受限章节材料。新闻、搜索、
Sitemap 和 RSS 继续完全隐藏受限项。

Wiki 组别分类只读取一级资料目录 `index.md` 当前 Revision 的 `tags`，固定为机械组、嵌入式组、
软件算法组、运营组和通用资料。标签规范化后投影到同一文档全部章节；非 index 标签不参与分类。
缺失、空数组或任何非法值统一公开为“未分类”。标签继续保存在既有 Frontmatter JSONB，并随
导入、发布、导出和恢复流转，不新增数据库列。完整规则见 `docs/WIKI_TAG_CLASSIFICATION.md`。
CMS 仍以每个 Markdown 页面作为 Article/Revision 粒度，但创建与列表按一级目录 `index.md`
呈现“主文档 + 章节”：主文档可编辑标签，章节选择所属主文档并继承其分类。创建时计划路径使用
草稿既有 JSONB 内部字段保存，发布生成正式 Front matter 时移除，不新增父子关系列。新主文档的
一级目录只能由真实日期和资料名称生成 `YYYY-MM-DD-资料名称`；前端只读预览并实时查重，服务端
使用同一校验再次生成路径，不能接受自由目录名。

团队当前组织结构是独立于成员档案的站点配置，公开入口位于 `/team/organization`，但
...
公开页以代码原生的嵌套动态星系呈现，官方 Logo 通过稳定机构 ID 的受控前端注册表映射。三机构
围绕固定且弱化尺寸的中性三方协作核心缓慢公转；机构行星与部门卫星作为信息主体使用更大的尺寸，
每个机构内部再由本机构的负责人职责和 `group` 部门组成局部
卫星轨道；同一轨道上的节点使用一致动画相位保持固定角间距。部门周围的小卫星使用独立外扩轨道
提示其下级 `role`。点击部门会打开局部聚焦舱，岗位沿部门轨道实际
运行；新闻部与运营组之间的柔和外弧按两个节点的实际运动位置实时连接，由低亮光晕、点状脉冲和
浅暖黄移动光点组成，并主动绕开中心；曲线依次经过三个不同相位摆动的波峰，使形状本身持续明显
变化。关系层位于全部机构、负责人和部门行星下方，穿过节点时由星球自然遮挡。默认不显示关系正文，点击
链路后才打开说明。机构、部门和负责人名称使用放大的高对比字号，深浅色分别使用完整星空色板；窄屏将嵌套
轨道降级为按机构分组的卡片并以居中浮层显示聚焦舱，CMS 预览和减少动态效果偏好停止公转。IRI 的
当前定位说明为“实验室”。公开组织页另使用独立 Vue WebGL 组件绘制 React Bits Galaxy 风格的
四层景深星流、耀光与闪烁；鼠标进入时星空围绕指针产生平滑排斥和穿梭拉伸，离开后恢复均匀星场。
组件只依赖轻量 `ogl` 封装 shader，未引入 React；深色主题输出亮星，浅色主题使用低耀光的冷灰蓝
细星尘与少量亮星核心，保持接近的星点数量但不形成大块蓝色光斑；浅色组织图使用更高对比度的
主轨道和机构局部轨道，仍保留实线、虚线与机构强调色的层级。桌面端上限 120 FPS，粗指针设备
上限 60 FPS，实际帧率不超过屏幕刷新率；指针平滑按实际帧间隔计算，排斥中心约 0.2 秒收敛且
不因刷新率改变速度。减少动态效果偏好只保留静态帧。背景不读取组织数据、不接管点击事件。
...
`/team/apply`。稳定账号 ID 等于成员档案稳定 ID，浏览器只读展示；冲突时请管理员处理，不加后缀。申请不会直接
创建账号，管理员在 CMS“审核”中的“账号注册申请”审核通过后，事务内创建普通 `member` 用户并写入唯一成员绑定。

`account_registration_applications` 仅在 pending 阶段保存 Argon2id 密码哈希，通过或拒绝后清除；
`user_members` 继续在内部保证一名成员只关联一个账号；CMS 不提供手动绑定入口。同源、IP 限流、管理员角色、CSRF 和审计边界保持
不变。具体流程与测试见 `docs/ACCOUNT_REGISTRATION.md`。

## 12. Footer 组织与合作信息（2026-08）

公开站 Footer 的机构 Logo 墙由 `app/data/footer-partners.ts` 集中维护，分为“指导与依托”、
“组织与实践平台”和“赛事与合作支持”，避免将所属高校、所属社团、实践平台或核心赛事误标为赞助商。
每项同时展示 Logo、中文名称和身份，整项跳转对应官方页面。Logo 使用
`cdn.sdutvinci.cn/site-assets/images/sponsors/` 下的透明 WebP，并直接叠放在 Footer 背景上；
前台将所有 Logo 合并为一条连续轮播带，鼠标悬停或键盘聚焦会暂停；重复副本不进入键盘焦点
或辅助技术列表。桌面端和移动端轮播项都根据 Logo 与名称的实际渲染宽度自动收缩，并统一保留最小
留白；移动端项间距还会随视口宽度在紧凑范围内调整，避免圆形标志被固定宽槽位拉开。减少动态效果
偏好会关闭动画，浅深色适配仍由代码仓库管理。
