...
公开页以代码原生的全页动态星系呈现，官方 Logo 通过稳定机构 ID 的受控前端注册表映射。三机构
围绕固定的“协同运行”核心缓慢公转，全部 `group` 作为可点击部门行星运行在更大的外环，并用岗位
小卫星提示其下级 `role`；点击部门会暂停外环并打开局部轨道聚焦舱，完整显示该部门及岗位。负责人
层仍居中强调机器人队队长。深浅色使用独立星空色板；窄屏将全局轨道降级为部门卡片并以居中浮层
显示聚焦舱，CMS 预览和减少动态效果偏好停止公转。IRI 的当前定位说明为“实验室”。
