...
## 2026-08-12：PR 导入分类筛选与 CMS 批量工作流

- PR Dry Run 分类统计改为可访问的筛选按钮；可人工覆盖的高风险项显示明确的“强制导入此
  高风险项”控件说明，仍要求原精确确认短语，未知语法、内容冲突、非法文件和路径不能覆盖。
- 草稿列表新增本人草稿批量提交审核；审核页新增待审核批量通过和“已通过，等待发布”批量
  正式发布。每批最多 100 篇，逐项复用单篇所有权、CSRF、编辑锁、乐观版本、状态和正式
  Revision 基线校验；部分失败独立报告并保持失败项原状态。
- 未新增数据库表或字段，不需要 Migration；没有修改独立内容仓库、正式 Markdown、Nuxt
  Content 或 Wiki 拼音/章节模块。实现和验证仅在本机隔离环境完成，没有 Push、部署、SSH、
  生产数据库或生产对象存储操作。

## 2026-08-12：CMS 批量工作流上限与有界并发补充

- 外部导入页新增“全选当前可导入结果”，仅选择当前组合筛选中仍可操作的 pending 项；取消
  全选只影响当前结果。草稿提交、审核通过和正式发布的请求上限由 100 统一提高到 500。
- 三种批量工作流使用最多 10 个 worker 并发复用原单篇事务，不会一次启动 500 个事务；结果
  仍按请求顺序返回，单项失败继续隔离，不回滚其他成功项。
- 无数据库字段变化，不创建 Migration；未修改 Wiki 路径模块或独立内容仓库。

## 2026-08-12：CMS 审核页流程化布局

- `/cms/reviews` 将待审核文章、已通过待发布文章和成员信息申请统一为三个连续流程面板；每个
  面板在同一标题栏显示阶段编号、当前数量、说明和本阶段操作，批量操作不再悬浮于页面右侧。
- 空数据时不再保留无内容的表格表头和大面积留白，改为带语义图标、标题和下一步说明的紧凑
  空状态；有数据时仍显示原表格、选择框、详情入口和成员申请卡片，业务流程没有变化。
- 桌面深浅色和 390px 窄屏均使用现有 CMS token；窄屏操作按钮自动收为单列，页面无横向溢出，
  数据表在真实有数据时继续允许横向滚动。
- 本功能只修改审核页模板和 CMS 样式，补充现行 PR 导入/成员申请说明与静态回归；不修改 API、
  权限、批量并发、Revision、Outbox、数据库 Schema 或独立内容仓库，不需要 Migration。

## 2026-08-12：CMS 活动草稿与已发布历史分离

- 工作台“活动草稿”总数和草稿页默认视图不再包含 `published`；已发布记录保留在独立的
  “已发布历史”视图，不删除数据库记录，也不改变 Revision、发布或导出数据。
- 已发布草稿详情明确提示只读。记录所有者可点击“继续编辑正式文章”，复用既有文章草稿入口将
...
## 2026-08-20：匿名 Wiki 成员资料区

- 匿名 Wiki 首页新增独立“成员资料”区域。完全受限的 Wiki 按 `docKey` 合并为一张锁定卡；点击
  进入登录页，登录后回到该 Wiki。公开 Wiki 若混有受限页面，则在公开文档卡内显示部分受限状态
  和登录入口，不重复生成完整受限卡。
- 匿名摘要只包含文档键、文档级标题、日期和登录后目标路径，不返回章节标题、章节数量、正文、
  Revision、哈希或搜索摘要。已登录列表继续返回完整目录，成员资料区和部分受限提示不再显示。
- Wiki 首页按文章级权限区分两种文档状态：没有公开页面的文档进入醒目的成员资料书架；仍有公开
  页面时保留在知识目录，显示“部分内容需登录”徽标、登录入口和可浏览文章数，不暴露受限章节标题
  或数量。首页同步重构英雄区、概览、搜索工具栏、双列文档卡、展开章节及移动端单列布局。
- 新闻列表、搜索、Sitemap 和 RSS 继续完全隐藏受限文章。无 Schema 变化，不创建 Migration，
  不修改独立内容仓库、正式 Markdown 或 Wiki 拼音/章节模块。

## 2026-08-20：Wiki 资料组别标签分类

- Wiki 分类固定为机械组、嵌入式组、软件算法组、运营组和通用资料；一份资料支持多个标签。只有
  一级资料目录 `index.md` 当前 Revision 的 `tags` 参与分类，章节或子目录中的标签不生效。
- 缺失、非数组、空数组、非字符串、固定集合外标签及合法/非法混合数组均明确归为“未分类”；
  合法标签去重并按固定顺序返回。公开 API 把文档级标签投影到全部章节，受限摘要也只增加该
  规范化分类，不返回受限章节材料。
- Wiki 首页新增带数量的分类导航，桌面为左侧面板、窄屏为横向按钮；分类写入 `tag` 查询参数并
  与搜索组合。公开卡片、完整受限卡片和详情页展示标签，多标签资料在各对应分类中均可找到，
  全部资料视图只显示一次。
- 标签继续由既有 `articles.frontmatter` / `article_revisions.frontmatter` JSONB 保存，并通过
  初次导入、PR 草稿、审核发布、确定性导出、snapshot 和恢复流转。无 Schema 变化，不创建
  Migration，不修改独立内容仓库、正式 Markdown 或 Wiki 拼音/章节工具。
- 使用、兼容和验收边界见 `docs/WIKI_TAG_CLASSIFICATION.md`。

## 2026-08-21：CMS Wiki 主文档与章节创建

- `/cms/articles/new` 将 Wiki 新建拆为“主文档”和“章节”。主文档在创建时确定一级目录并固定为
  `<目录>/index.md`，可多选五类组别标签；章节必须选择已有主文档，并在其目录下填写非
  `index.md` 的章节文件名。
- CMS 正式内容列表按一级目录重排：`index.md` 显示为主文档并排在所属章节之前，章节显示
  “属于：主文档标题”；历史单文件 Wiki 明确显示“旧式独立页”，不伪装为可分类主文档。
- 每个 Markdown 仍是独立 Article/Revision，以保留逐页版本、权限、审核和恢复。创建时计划路径
  暂存于草稿既有 Frontmatter JSONB 内部字段，正式发布时固定使用并剔除该字段；无需新增父子
  关系表或 Migration。
- 主文档草稿的“文章信息”可继续调整标签，章节不提供标签编辑且拒绝通过保存 API 注入标签；
  审核页显示主文档正式/草稿标签和只读计划路径。旧草稿、旧 Revision 与非 index 标签原文继续
  兼容。

## 2026-08-21：CMS Wiki 主文档规范目录生成

- `/cms/articles/new` 不再让编辑者自由填写一级目录。主文档改为分别选择资料日期和填写不含日期
  的资料名称，并在下方显示不可编辑但可选择复制的 `YYYY-MM-DD-资料名称` 及完整 `index.md`
  计划路径；符合该格式的文章标题可自动拆分为两个字段。
- 日期必须是规范且真实的公历日期；名称不能为空，不能包含路径/控制字符，也不能重复日期前缀。
  页面实时显示格式状态并检查正式内容和活动草稿占用，提交接口仍在服务端按同一共享规则生成并
...
## 2026-08-21：组织架构动态星系与机构 Logo

- 公开组织图的三机构节点使用 Vinci、EMIS 和 IRI Lab 官方 Logo；素材由稳定机构 ID 的受控前端
  注册表映射，不开放任意图片 URL，也不写入组织架构 JSON。
- 桌面端三机构沿同一轨道以 72 秒周期围绕固定“协同运行”核心缓慢公转，文字保持正向；悬停或
  键盘聚焦机构时整体暂停。负责人层保持固定，机器人队队长继续位于正中并突出显示。
- CMS 紧凑预览、720px 以下窄屏和 `prefers-reduced-motion` 使用静态语义卡片；页面仍可完整读取，
  不依赖 Canvas 或静态组织图图片。
- 本轮只调整展示层、静态回归和现行文档，不修改组织架构 API、数据库 Schema、成员数据或内容
  仓库，因此不新增 Migration。

## 2026-08-21：全页组织星系与部门聚焦

- 公开组织架构从“中心星系 + 两侧普通部门列”重构为全页星系：三机构沿内层轨道运行，全部部门
  作为可点击行星沿外层大环运行，岗位数量通过部门周围的小卫星提示，跨部门关系继续使用彗星轨迹。
- 点击部门会暂停外环、弱化其他节点，并打开局部轨道聚焦舱；例如机械组会围绕中心部门星完整显示
  机械组组长、机械1组和机械2组。没有下级岗位时明确提示，关闭按钮与 `Esc` 均可退出聚焦。
- 深浅色改用独立星空变量，机构 Logo 只放在小型中性托盘内，消除深色模式的大块白底和低对比文字。
  窄屏将全局轨道降级为可读部门卡片，点击后使用居中浮层；CMS 紧凑预览继续使用静态布局。
- 本轮只修改组织架构组件、样式、静态回归与现行文档；不修改 API、组织数据、数据库 Schema、成员
  数据、独立内容仓库、正式 Markdown 或 Wiki 路径工具，因此不新增 Migration。

## 2026-08-21：机构局部星系与可交互关系链路

- 组织图从“全部部门共用一个外环”调整为嵌套星系：三个机构围绕协同核心公转，每个机构内部仅由
  自己的负责人职责和部门卫星构成局部轨道。社团会长、机器人队队长和基地实验室负责人分别回到
  EMIS、Vinci 和 IRI Lab 的局部系统，不再悬空集中展示。
- 部门下的岗位先以小卫星提示；点击部门后打开局部聚焦舱，岗位球与名称沿部门椭圆轨道持续运行。
  机械组可完整看到机械组组长、机械1组和机械2组；无岗位部门仍明确提示未配置。
- 新闻部与运营组的孵化关系改为动态闪电轨迹和小字按钮；点击后打开独立关系框，显示起点、终点和
  完整关系说明。窄屏与 CMS 紧凑预览按机构降级为静态语义卡片，部门与关系弹层仍可操作。
- 本轮只修改组织架构展示组件、样式、静态回归与现行文档；不修改组织数据、API、数据库 Schema、
  成员数据、独立内容仓库、正式 Markdown 或 Wiki 路径工具，因此不新增 Migration。

## 2026-08-21：组织星系可读性与动态关系线

- 公开图不再渲染“通常由同一人兼任”以及机构 division 说明；底层字段继续兼容旧数据和 CMS 编辑，
  但不会占用星图空间。中心图形改为中性的三节点协作符号，不使用学校 Logo，避免暗示三机构存在
  数据模型之外的直属关系。
- 新闻部与运营组的关系线根据两个真实卫星节点的当前屏幕位置持续更新，随公转一起移动；线条改为
  绕开中心的柔和外弧，由低亮光晕、点状脉冲和移动光点构成。默认只显示轻量链路，点击线条后才
  展示关系正文；SVG 焦点采用线条自身高亮，不再出现随路径边界变化的矩形轮廓。窄屏和 CMS 紧凑
  预览保留显式“查看跨部门关系”入口。
- 关系曲线改为经过三个不同相位动态波峰的平滑样条；在端点因键盘聚焦而暂停时，实测端点坐标保持
  不变，720ms 内中间波形坐标最大变化约 55px。关系层降到机构星系下方，由星球自然遮挡；主色改为
  浅薄荷绿，移动亮点使用浅暖黄。三个负责人职责卫星改为可点击按钮，并提供所属机构职责聚焦舱。
- 机构 Logo 不再叠加白色托盘；Vinci 现有 WebP 已带 Alpha 通道，无需生成近似 Logo。协作核心
  缩小，机构行星与部门卫星放大。深浅色为星图、行星、弹层和文字分别设置完整高对比色板，并放大
  机构、部门、负责人及聚焦舱关键文字。
- 机构、部门和负责人卫星取消彼此不同的负动画延迟，改为等角距同步公转；机器人队队长与运营组
...
## 2026-08-21：Galaxy 跟手性与浅色星场修正

- 30 FPS 渲染继续保留，但指针坐标插值从每帧 `0.05` 提高到 `0.32`，约 0.2 秒收敛到新位置；排斥
  激活使用 `0.24` 快速进入，离开使用 `0.12` 缓动恢复，避免鼠标移动后星流中心明显拖尾。
- 浅色模式不再把完整深色耀光直接染成高 Alpha 深蓝。浅色专用参数提高密度、降低 Glow 与闪烁；
  shader 以低 Alpha 细星尘层和少量亮星核心合成，峰值 Alpha 限制为 `0.3`，星点数量接近深色模式但
  不产生大块蓝色污点；CSS 静态回退也同步降低不透明度。
- 本轮只修改背景组件、回退样式、静态测试与现行文档；不修改组织数据、API、数据库 Schema、成员、
  正式 Markdown、独立内容仓库或 Wiki 路径工具，因此不新增 Migration。

## 2026-08-21：Galaxy 高帧率调度

- WebGL 星流不再使用固定 `33ms` 的约 30 FPS 硬限流。桌面端上限提升为 120 FPS，粗指针设备上限
  提升为 60 FPS；实际绘制由 `requestAnimationFrame` 约束在屏幕刷新率内，60Hz 桌面仍只绘制约
  60 帧，120Hz 桌面才会达到约 120 帧。
- 鼠标位置及排斥激活的平滑系数改为根据实际绘制间隔计算，维持约 0.2 秒的收敛时间，避免刷新率
  提升后动画速度翻倍。页面隐藏暂停、减少动态效果静态帧和 165 万渲染像素上限继续保留。
- 浅色组织图的主椭圆、内层轨道、信号虚线和机构局部轨道使用独立的较高对比色；局部轨道继续混合
