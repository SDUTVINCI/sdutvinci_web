...
- 阶段 3 实现、迁移和自动化验证已完成，等待维护者人工验收。
- 阶段 3 任务、范围约束和验收标准已勾选；总体进度及通用模板“等待本阶段验收通过”保持未勾选。
...
- 进入可视化模式前检查 HTML/Vue、Jekyll 模板和 MDC 扩展；检测到未知语法时只允许源码模式。
- 可视化编辑器初始化后比较序列化结果；往返结果不一致就恢复原文并返回源码模式。
...
- 隔离 PostgreSQL 测试库：3 个测试文件、15 项测试全部通过。
...
4. 在安全的普通 Markdown 中切换可视化和源码模式，确认内容不丢失。
5. 选择包含 `<NuxtLink>` 的 Wiki 文章创建草稿，确认系统拒绝进入可视化模式并保留源码。
