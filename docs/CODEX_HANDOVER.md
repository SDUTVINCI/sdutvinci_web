...
> 历史基线：本文件记录 V1 实施与验收，保留用于审计，不是 V2.0 现行运维手册。
> 当前操作请使用 `docs/DEPLOYMENT.md` 和 `docs/v2/OPERATIONS.md`。
...
## 2026-07-26：阶段 9 人工验收完成

- 维护者明确确认“阶段 9 验收通过”。
- 需求文档中的阶段 9 总体进度已勾选；阶段 0～9 现已全部完成。
- `docs/PHASE9_SECURITY_AND_ACCEPTANCE.md` 已更新最终验收结论，并记录阶段 9
  实现、依赖安全修复和部署缓存清理提交。
- 原始 Markdown HTML 的存储型 XSS 风险继续按已确认策略接受；没有在收尾时
  改变内容渲染兼容性。
- 阶段 1～8 功能、Docker Compose 双槽位、gateway、内网主动拉取、content 与
  application 两条通道，以及备份、恢复和迁移安全边界均未改变。
- 本次只修改验收文档，没有代码、数据库、API、配置或部署架构变更；没有 push
  GitHub、连接服务器或触发外部发布。

## 2026-07-26：验收后新增 CMS 账号管理

- `/cms/users` 对所有已登录用户开放：普通成员只能查看自己的账号安全信息并在
  验证当前密码后修改本人密码；管理员可创建账号，维护全部账号的角色和状态，
  并重置其他账号密码。
- 普通成员没有目标用户 ID 写入口；管理员列表、创建和更新 API 继续在服务端
  强制 `admin`，页面中的角色判断只用于显示，不作为权限边界。
- 本人改密保留当前会话并撤销其他活动会话；管理员重置密码撤销目标账号全部
  活动会话。审计只记录密码发生变更，不记录原始密码或密码哈希。
- 管理员不能通过重置入口绕过自己当前密码的验证。最后一名有效管理员保护已
  下沉到带 PostgreSQL advisory transaction lock 的更新事务，避免并发降权或
  停用同时通过检查。
- 创建账号时若已存在同稳定 ID 的 member，会在同一事务立即建立一对一关联，
  使导航栏和后台直接显示成员姓名与头像。
- 未增加 migration，现有账号、角色、会话和 member 关联结构保持兼容。隔离
  `TEST_DATABASE_URL` 下 CMS 8 个测试文件、41 项全部通过；production HTTP
  回归确认管理员列表/创建 200、普通成员越权 403、错误当前密码 400、本人改密
  200、旧密码 401、新密码 200。类型检查和 production build 通过；未连接正常
  数据库、生产服务器或真实凭据，未 push 或部署。
