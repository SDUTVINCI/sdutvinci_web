...
- 新闻列表完全隐藏需登录文章；Wiki 正常目录不返回受限页面，但会在独立“队内资料”区域显示
  合并后的文档级锁定卡片；
...
匿名 Wiki 的“队内资料”按 `docKey` 合并，同一文档无论包含多少受限章节都只显示一张卡。卡片只
显示文档级标题和“需登录查看”，不返回或展示章节标题、章节数量、正文、Revision、哈希或搜索摘要。
如果同一 Wiki 同时含公开与受限页面，公开卡片只增加“含需登录文章”标识，不重复生成锁定卡。
登录后 API 返回完整可访问目录，独立锁定区自然消失。文档级标题会向匿名用户公开，因此标题本身
也敏感的资料不应依赖该展示方式，应使用不泄密的文档标题或采用更严格的独立存储与访问控制。
...
4. 对受限 Wiki，使用无会话浏览器确认它不在普通目录和章节列表，但按文档合并显示在“队内资料”
   中；卡片不显示章节名、数量或正文。受限新闻仍完全不出现在新闻列表。
