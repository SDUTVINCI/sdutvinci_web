...
const port = Number(process.env.PHASE8_MANUAL_MOCK_PORT || '0')
const token = process.env.CONTENT_PR_IMPORT_GITHUB_TOKEN || ''
if (!stateRoot.endsWith('/vinci-v2-phase8-manual-test') || !port || !token) {
  throw new Error('PHASE8_MOCK_CONFIGURATION_INVALID')
}
...
const authorized = (request: import('node:http').IncomingMessage) =>
  request.headers.authorization === `Bearer ${token}`
const body = async (request: import('node:http').IncomingMessage) => {
  let source = ''
  for await (const chunk of request) {
    source += String(chunk)
    if (source.length > 32_768) throw new Error('BODY_TOO_LARGE')
  }
...
createServer(async (request, response) => {
  try {
    ...
    if (request.method === 'GET' && url.pathname === '/health') {
      return json(response, 200, { status: 'ok', scope: 'v2-phase8-manual-test' })
    }
    if (!authorized(request)) return json(response, 401, { message: 'mock authorization required' })
    const prefix = `/repos/${fixture.repositoryId}`
    if (request.method === 'GET' && url.pathname === `${prefix}/pulls/${fixture.pullRequestNumber}`) {
      const pullState = JSON.parse(await readFile(stateFile, 'utf8')) as { state: string }
    ...
    if (request.method === 'GET' && url.pathname.startsWith(contentPrefix)) {
      ...
      if (![fixture.baseCommit, fixture.headCommit].includes(ref)) return json(response, 404, {})
      const path = decodeURIComponent(url.pathname.slice(contentPrefix.length))
      if (!path || path.includes('..') || path.includes('\\') || path.includes('\0')) return json(response, 422, {})
    ...
    if (request.method === 'POST'
      && url.pathname === `${prefix}/issues/${fixture.pullRequestNumber}/comments`) {
      const input = await body(request) as { body?: string }
    ...
    if (request.method === 'PATCH' && url.pathname === `${prefix}/pulls/${fixture.pullRequestNumber}`) {
      const input = await body(request) as { state?: string }
    ...
    return json(response, 500, { message: 'mock failure' })
  }
