...
describe('独立组织架构', () => {
  ...
  it('公开页和 CMS 只复用架构组件，不引入成员选择器或历史赛季', async () => {
    ...
    expect(chart).toContain('organization-department-system')
    ...
    expect(chartStyles).toContain('@keyframes organization-department-orbit')
    expect(chartStyles).toContain('.organization-department-system.has-focus')
    expect(chartStyles).toContain('.organization-department-focus')
