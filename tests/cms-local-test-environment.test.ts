...
describe('CMS 一键本地测试环境', () => {
  it('固定隔离资源、完整夹具、测试管理员和安全清理边界', async () => {
    const [shell, fixture, guide] = await Promise.all([
    ...
    expect(shell).toContain('-d node:24-bookworm')
    ...
    expect(fixture).toContain('fixtures.length !== 228')
    expect(fixture).toContain('applyCmsMemberMarkdownMigration()')
    expect(guide).toContain('articles=228,members=32')
