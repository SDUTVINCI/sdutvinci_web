...
describe('成员选项、自动归类与 Markdown 图床', () => {
  ...
  it('使用学校教学单位作为学院选择题，并以浏览器样式导出 A4 PDF', () => {
    ...
    expect(MEMBER_COLLEGE_OPTIONS.indexOf('智能农业装备工程学院'))
      .toBe(MEMBER_COLLEGE_OPTIONS.indexOf('农业工程与食品科学学院') + 1)
  ...
  it('公开申请与 CMS 创建复用成员资料表单，并区分参赛和指导届次', async () => {
    ...
    expect(avatar).toContain('class="member-required-marker">必填</strong>')
    expect(avatar).toContain('（文件名自动转为姓名-哈希.webp）')
    expect(avatar).toContain('头像预览')
    expect(avatar).toContain("'member-avatar-name-warning': !name.trim()")
    ...
    expect(avatar).toContain('if (wasUploading && !uploading) clearLocalPreview()')
