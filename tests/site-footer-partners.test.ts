...
describe('网站 Footer 合作与支持', () => {
  ...
  it('整项 Logo 可点击，显示中文名称和身份，并提供紧凑响应式布局', async () => {
    ...
    expect(component).toContain("String(groupIndex + 1).padStart(2, '0')")
    expect(component).not.toContain('{{ group.eyebrow }}')
    ...
    expect(footerStyles).toMatch(/\.footer-partner-group\s*\{[^}]*grid-template-columns:\s*190px minmax\(0, 1fr\);/)
    expect(footerStyles).toMatch(/\.footer-partner-group-heading h3\s*\{[^}]*font-size:\s*1\.3rem;/)
    expect(footerStyles).toMatch(/\.footer-partner-group-school \.footer-partner-logo-school\s*\{[^}]*max-width:\s*360px;/)
