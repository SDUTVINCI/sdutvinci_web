...
describe('网站 Footer 合作与支持', () => {
  ...
  it('整项 Logo 可点击，显示中文名称和身份，并提供紧凑响应式布局', async () => {
    ...
    expect(responsiveStyles).toMatch(/@media \(max-width: 620px\)[\s\S]*\.footer-partner-set\s*\{[^}]*gap:\s*clamp\(12px, 3vw, 18px\);[^}]*padding-right:\s*clamp\(12px, 3vw, 18px\);/)
    expect(responsiveStyles).toMatch(/@media \(max-width: 620px\)[\s\S]*\.footer-partner-card\s*\{[^}]*width:\s*fit-content;[^}]*min-width:\s*104px;[^}]*max-width:\s*290px;/)
    expect(responsiveStyles).not.toMatch(/\.footer-partner-card-(?:school|organizations|partners)\s*\{[^}]*width:/)
  })
})
